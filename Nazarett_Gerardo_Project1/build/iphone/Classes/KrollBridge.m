...
NSString * Nazarett_Gerardo_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Nazarett_Gerardo_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_nazarett_gerardo_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Nazarett_Gerardo_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Nazarett_Gerardo_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_nazarett_gerardo_project1 = [[Nazarett_Gerardo_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_nazarett_gerardo_project1];
	
	NSString *_nazarett_gerardo_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _nazarett_gerardo_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_nazarett_gerardo_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Nazarett_Gerardo_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.nazarett_gerardo_project1.kroll" 
