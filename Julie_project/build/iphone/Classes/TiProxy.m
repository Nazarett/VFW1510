...
//Rare exceptions to indicate a bug in the _julie_project code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_julie_project = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_julie_project]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.julie_project.module"
